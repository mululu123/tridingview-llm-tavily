...
用户输入股票代码（如 000001）
         ...
         ├──> TradingView MCP（并行）
         │     ├── get_quote()        → 实时报价
         │     ├── get_price(1D/60/30min, range=100)  → 多周期K线
         │     └── get_ta(include_indicators=true)     → RSI/MACD/KDJ/布林带
         │
         ├──> Tavily Search（并行）
...
| 数据源 | 用途 | 调用方式 |
|--------|------|---------|
| **TradingView MCP** | K线、技术指标、实时报价 | 通过 Claude Code MCP 调用 |
| **Tavily API** | 个股新闻、市场热点搜索 | REST API（tvly search） |
| **Claude API** | 综合分析、评分、摘要 | Anthropic SDK (anthropic) |
...
AI：  anthropic SDK（pip install anthropic）
搜索：tavily-python（pip install tavily-python）
数据：TradingView MCP（通过 Claude Code 调用）
存储：无（MVP 不保存历史）或 SQLite 单文件（后续扩展）
运行：python app.py → 浏览器自动打开 localhost:8501
```
...
├── services/
│   ├── tradingview.py      # TradingView MCP 数据获取
│   ├── tavily_search.py    # Tavily 搜索封装
│   └── claude_analyzer.py  # Claude 分析 & Prompt 模板
├── prompts/
│   ├── stock_analysis.md   # 个股分析 Prompt
